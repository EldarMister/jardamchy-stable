...
OBLAST_CARGO_FILLERS = frozenset({
    # грузовые маркеры (сами объекты, не карго)
    "жүк", "жук", "груз", "жеткир", "жеткирүү", "жеткируу", "жеткирүш",
    "доставка", "доставить",
    # базовые глаголы движения/передачи
    "алып", "ал", "алыш", "алуу",
    "бер", "берүү", "беруу", "берип", "бериш", "берүш",
    "кел", "келүү", "келуу",
    "бар", "баруу",
    "ташыш", "ташуу",
    # отправка / доставка (жибер-)
    "жибер", "жибериш", "жиберүү", "жиберуу", "жибергин", "жибергиле",
    # отправка / отправление (жонот- / жөнөт-)
    "жонот", "жонотуш", "жонотуу",
    "жөнөт", "жөнөтүш", "жөнөтүү", "жонотуш",
    # вспомогательные глаголы
    "салып", "салыш",
    # "надо/нужно" варианты (кыргызский)
    "керек", "кирек", "кереk",
    "нужно", "надо", "please",
    # предлоги/союзы
...
def _extract_oblast_suffix_addresses(message: str) -> tuple[str, str]:
    ...
    for i, token in enumerate(tokens):
        ...
        if not resolved:
            ...
            # If canonical found a true match (different from the raw lowercase base),
            # use it; otherwise just capitalize the base for display
            resolved = canonical if (canonical and canonical.lower() != base) else base.capitalize()
        if direction == "to":
            tos.append(resolved)
        else:
