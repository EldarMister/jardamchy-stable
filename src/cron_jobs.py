...
def _send_mention_to_group(chat_id: str, order_id: str, service_type: str, db) -> None:
    """Построить строку упоминаний и отправить в группу."""
...
def check_mention_reminders():
    """Если заказ не принят за 30 секунд — первое упоминание исполнителей в группе."""
    try:
        db = get_db()
        ...
        for timer in timers:
            ...
            _send_mention_to_group(chat_id, order_id, service_type, db)
...
def check_second_mention_reminders():
    """Если заказ не принят за 2 минуты — повторное упоминание исполнителей."""
    try:
        db = get_db()
        timers = db.get_timers_needing_second_mention(delay_seconds=120)
        ...
        for timer in timers:
            ...
            if not chat_id:
                db.mark_second_mention_sent(timer_id)
                continue

            if not db.mark_second_mention_sent(timer_id):
                continue
        ...
        logger.exception("Error checking second mention reminders")
        return False
...
def run_all_cron_jobs():
    ...
    check_second_mention_reminders()
    check_cafe_timeouts()
    check_taxi_timeouts()
