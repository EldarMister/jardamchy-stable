...
💻 *8. Компьютерные услуги* - Установка / настройка
🚘 *9. Попутка* - Поиск попутки
🔧 *10. Сантехника* - Вызов мастера
...
COMPUTER_SERVICES_MESSAGE = """💻 *Компьютерные услуги / Компьютер кызматтары*

*Биздин кызматтар:*

*1. Полиграфия*
Визиткадан көлөмдүү конструкцияларга чейин сапаттуу басып чыгаруу.

*Кызматтар тизмеси:*
• Визиткалар
• Баннерлер
• Өзүнөн өзү желимдегич
• Көлөмдүү тамгалар
• DTF печать

*2. Сүрөт жана видео*
Бизнес, жарнама жана маанилүү иш-чаралар үчүн профессионалдык тартуу.
...
📞 *Байланыш номери:* 220 122 232

*3. Бизнести автоматизациялоо*
Кафе, ресторандар жана дүкөндөр үчүн автоматизация системаларын иштеп чыгып, ишке киргизебиз.

*Кызматтар тизмеси:*
• CRM — кардарлар жана сатуу бир жерде башкарылат
• SaaS платформалар — бизнес үчүн ыңгайлуу чечимдер
• Заказдарды автоматизациялоо — менеджерсиз заказдарды кабыл алуу жана иштетүү

*4. Сайттар*
Заманбап, ылдам жана ыңгайлуу сайттарды жасайбыз.

📞 *Байланыш номери:* 226 410 410

Жазыңыз, кайсы кызмат керек экенин — багыттап беребиз."""

POPUTKA_MESSAGE = """🚘 *Попутка*

Напишите маршрут, время и сколько мест нужно.

Оператор поможет найти попутку.
📞 {support_phone}"""
...
Сейчас добавим ваш ближайший выезд в WhatsApp-список.

Напишите, *откуда* отправляетесь.
...
Сколько еще человек можете взять?
...
Во сколько отправляетесь *сегодня*?

Формат времени: *HH:MM*
Пример: 19:00"""

POPUTKA_DRIVER_SUCCESS = """✅ *Попутка добавлена*

📍 Откуда: {from_address}
📍 Куда: {to_address}
👥 Мест: {seats}
🕒 Выезд: {departure_time}

Теперь эта запись видна клиентам в WhatsApp до времени отправления."""

POPUTKA_DRIVER_INVALID_TIME = """⚠️ Укажите время в формате *HH:MM* и только на сегодня.

Пример: 19:00"""

POPUTKA_DRIVER_INVALID_SEATS = """⚠️ Напишите количество мест числом.

Пример: 2"""

POPUTKA_LIST_EMPTY = """🚘 *Попутка*

Сейчас нет водителей, которые скоро отправляются.

Попробуйте позже."""

POPUTKA_LIST_HEADER = """🚘 *Ближайшие попутки*

Ниже водители, которые скоро выезжают:"""
