...
// ============================================================================
// CONTACTS (MASTERS + MED EJE) — combined section
// ============================================================================

async function loadContacts() {
    await Promise.all([_loadMastersTable(), _loadMedEjeTable()]);
}
...
async function _loadMedEjeTable() {
    const body = document.getElementById('med-eje-body');
    try {
        const data = await api('/med-eje');
        const rows = data.contacts || [];
        body.innerHTML = rows.length === 0
            ? '<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:20px;">Нет контактов</td></tr>'
            : rows.map((c, idx) => `
            <tr>
                <td>${idx + 1}</td>
                <td>${esc(c.name)}</td>
